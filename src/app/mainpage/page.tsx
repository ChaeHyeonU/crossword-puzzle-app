...
export default function GameMainPage() {
  ...
  return (
    <main className="min-h-screen flex flex-col items-center justify-start bg-gradient-to-b from-blue-50 to-slate-100 dark:from-slate-900 dark:to-slate-800 text-slate-900 dark:text-slate-100 px-2 py-6">
      {/* 상단: 앱명, 슬로건, 지도 이모지 */}
      <div className="w-full max-w-lg flex flex-col items-center gap-2 mb-4">
        ...
        <p className="text-base text-slate-600 dark:text-slate-300 font-medium">재미있게 배우는 지리 개념, 크로스워드로 도전하세요!</p>
      </div>
      {/* 학습 현황/추천/기록/업적 */}
      <div className="w-full max-w-lg flex flex-col md:flex-row gap-3 mb-6">
        <div className="flex-1 bg-white/80 dark:bg-slate-800 rounded-xl p-4 flex flex-col items-center shadow border border-slate-200 dark:border-slate-700">
      ...
      <div className="w-full max-w-lg flex flex-row gap-3 mb-6">
        <div className="flex-1 bg-white/80 dark:bg-slate-800 rounded-xl p-4 flex flex-col items-center shadow border border-slate-200 dark:border-slate-700">
      ...
      {/* 섹션별 카드 */}
      <div className="w-full max-w-lg bg-white/90 dark:bg-slate-800 rounded-2xl shadow-xl p-6 flex flex-col gap-6 border border-slate-200 dark:border-slate-700">
        {/* 상단 부제목 */}
        <div className="flex items-center justify-between mb-4">
          <Button variant="ghost" onClick={handlePrev} disabled={current === 0} aria-label="이전 부제목으로">
            ◀
          </Button>
          <h2 className="text-xl font-bold text-center flex-1 flex items-center justify-center gap-2">
      ...
      {/* 하단 안내 및 버튼 */}
      <div className="w-full max-w-lg flex flex-col items-center gap-2 mt-8">
